...
namespace Mix.Cms.Lib.Helpers
{
    public class QueryFilterHelper
    {
        ...
        {
            Enum.TryParse(functionModel.Rule, out MixExpressionRule rule);
            return rule switch
            {
                MixExpressionRule.Eq => m => m.AttributeFieldName == functionModel.FieldName
                && m.StringValue == functionModel.Value,

                MixExpressionRule.Neq => m => m.AttributeFieldName == functionModel.FieldName
                && m.StringValue != functionModel.Value,

                MixExpressionRule.Ct => m => m.AttributeFieldName == functionModel.FieldName
                && (EF.Functions.Like(m.StringValue, $"{functionModel.Value}")),

                MixExpressionRule.Nct => m => m.AttributeFieldName == functionModel.FieldName
                && !(EF.Functions.Like(m.StringValue, $"{functionModel.Value}")),

                MixExpressionRule.Ra => m => m.AttributeFieldName == functionModel.FieldName
                && (string.Compare(m.StringValue, functionModel.MinValue) > 0
                && string.Compare(m.StringValue, functionModel.MaxValue) < 0),

                MixExpressionRule.Nra => m => m.AttributeFieldName == functionModel.FieldName
                && (string.Compare(m.StringValue, functionModel.MinValue) < 0
                || string.Compare(m.StringValue, functionModel.MaxValue) > 0),

                MixExpressionRule.Gte => m => m.AttributeFieldName == functionModel.FieldName
                && (string.Compare(m.StringValue, functionModel.Value) > 0
                || m.StringValue == functionModel.Value),

                MixExpressionRule.Gt => m => m.AttributeFieldName == functionModel.FieldName
                && string.Compare(m.StringValue, functionModel.Value) > 0,

                MixExpressionRule.Lte => m => m.AttributeFieldName == functionModel.FieldName
                && (string.Compare(m.StringValue, functionModel.Value) < 0
                || m.StringValue == functionModel.Value),

                MixExpressionRule.Lt => m => m.AttributeFieldName == functionModel.FieldName
                && string.Compare(m.StringValue, functionModel.Value) < 0,

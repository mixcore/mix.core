...
namespace Mix.MCP.Lib.Agents
{
    ...
    public class RetrievalAgent
    {
        ...
        /// <summary>
        /// Initializes a new instance of the RetrievalAgent class
        /// </summary>
        public RetrievalAgent(
            ILlmServiceFactory llmServiceFactory,
            ILogger<RetrievalAgent> logger)
        {
        ...
        public async Task<string> DatabaseAgent(
            string userInput,
...
""", toolList, userInput);
            ...
            {
                _logger.LogError(ex, "Failed to parse LLM intent classification response: {Content}", response?.choices?.FirstOrDefault()?.Message?.Content);
                return (DatabaseIntent.Unknown, new Dictionary<string, string>());
            }
        }

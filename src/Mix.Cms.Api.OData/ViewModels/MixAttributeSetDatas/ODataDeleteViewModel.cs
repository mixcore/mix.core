...
namespace Mix.Cms.Api.OData.ViewModels.MixAttributeSetDatas
{
      ...
      : ODataViewModelBase<MixCmsContext, MixAttributeSetData, ODataDeleteViewModel>
    {
        ...
        public override RepositoryResponse<bool> RemoveRelatedModels(ODataDeleteViewModel view, MixCmsContext _context = null, IDbContextTransaction _transaction = null)
        {
            ...
            // Remove values
            var removeValues = Lib.ViewModels.MixAttributeSetValues.DeleteViewModel.Repository.RemoveListModel(false, f => f.DataId == Id && f.Specificulture == Specificulture, _context, _transaction);
            ...
            if (result.IsSucceed)
            {
                var removeChildFields = Lib.ViewModels.MixAttributeSetValues.DeleteViewModel.Repository.RemoveListModel(false, f => (f.DataId == Id) && f.Specificulture == Specificulture, _context, _transaction);
                ViewModelHelper.HandleResult(removeChildFields, ref result);
                var removeChilds = Lib.ViewModels.MixAttributeSetDatas.DeleteViewModel.Repository.RemoveListModel(false, f => (f.Id == Id) && f.Specificulture == Specificulture, _context, _transaction);
        ...
        public override async System.Threading.Tasks.Task<RepositoryResponse<bool>> RemoveRelatedModelsAsync(ODataDeleteViewModel view, MixCmsContext _context = null, IDbContextTransaction _transaction = null)
        {
            var result = new RepositoryResponse<bool>() { IsSucceed = true };
            // Remove values
            var removeFields = await Lib.ViewModels.MixAttributeSetValues.DeleteViewModel.Repository.RemoveListModelAsync(false, f => f.DataId == Id && f.Specificulture == Specificulture, _context, _transaction);
            ...
            if (result.IsSucceed)
            {
                var removeChildFields = await Lib.ViewModels.MixAttributeSetValues.DeleteViewModel.Repository.RemoveListModelAsync(false, f => (f.DataId == Id) && f.Specificulture == Specificulture, _context, _transaction);
                ViewModelHelper.HandleResult(removeChildFields, ref result);
                var removeChilds = await Lib.ViewModels.MixAttributeSetDatas.DeleteViewModel.Repository.RemoveListModelAsync(false, f => (f.Id == Id) && f.Specificulture == Specificulture, _context, _transaction);
                ViewModelHelper.HandleResult(removeChilds, ref result);
            }
            return result;
        }

...
using static Mix.Cms.Service.SignalR.Constants;
    ...
    public class MixChatHub : BaseSignalRHub
    {
        ...
        private void SendToConnection<T>(T message, MessageReponseKey action, string connectionId, bool isMySelf)
        {
            if (!string.IsNullOrEmpty(connectionId))
            {
                ...
                if (isMySelf)
                {
                    Clients.Client(connectionId).SendAsync(MixHubMethods.ReceiveMethod, JObject.FromObject(result));
                }
                else
                {
                    Clients.OthersInGroup(connectionId).SendAsync(MixHubMethods.ReceiveMethod, JObject.FromObject(result));
                }
            }
        }
        ...
        private void SendToCaller<T>(T message, MessageReponseKey action)
        {
            ...
            Clients.Caller.SendAsync(MixHubMethods.ReceiveMethod, JObject.FromObject(result));
        }
        ...
        private void SendToGroup<T>(T message, MessageReponseKey action, string groupName, bool isMySelf)
        {
            if (!string.IsNullOrEmpty(groupName))
            {
                ...
                if (isMySelf)
                {
                    Clients.Group(groupName).SendAsync(MixHubMethods.ReceiveMethod, JObject.FromObject(result));
                }
                else
                {
                    Clients.OthersInGroup(groupName).SendAsync(MixHubMethods.ReceiveMethod, JObject.FromObject(result));
                }
            }
        }
        ...
        private void SendToAll<T>(T message, MessageReponseKey action, bool isMySelf)
        {
            ...
            if (isMySelf)
            {
                Clients.All.SendAsync(MixHubMethods.ReceiveMethod, result);
            }
            else
            {
                Clients.Others.SendAsync(MixHubMethods.ReceiveMethod, result);
            }
        }
        ...
        */
        ...
        public override Task OnDisconnectedAsync(Exception exception)
        {
            ...
            {
                getUserDevice.Data.Status = MixDeviceStatus.Disconnected;
                var countOnlineDevice = Service.SignalR.ViewModels.MixMessengerUserDevices.DefaultViewModel.Repository.Count(
                    d => d.UserId == getUserDevice.Data.UserId && d.DeviceId != getUserDevice.Data.DeviceId && d.Status == 1).Data;

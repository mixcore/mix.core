...
namespace Mix.Cms.Api.Controllers.v1
{
    ...
    {
        public ApiAttributeSetDataPortalController(MixCmsContext context, IMemoryCache memoryCache, Microsoft.AspNetCore.SignalR.IHubContext<Mix.Cms.Service.SignalR.Hubs.PortalHub> hubContext) : base(context, memoryCache, hubContext)
        {
        }
            ...
            [FromBody] RequestPaging request)
        {
            ...
            string attributeSetName = queries.Get("attributeSetName");
            ParseRequestPagingDate(request);
            var data = await Helper.FilterByKeywordAsync<ImportViewModel>(_lang, attributeSetName,
                        request, request.Keyword, queryDictionary);

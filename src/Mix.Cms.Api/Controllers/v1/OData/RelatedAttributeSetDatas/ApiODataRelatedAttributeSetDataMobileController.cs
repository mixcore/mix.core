...
namespace Mix.Cms.Api.Controllers.v1.OData.RelatedAttributeSetDatas
{
    ...
    [Route("api/v1/odata/{culture}/related-attribute-set-data/mobile")]
    //[Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "SuperAdmin, Admin")]
    public class ApiODataRelatedAttributeSetDataMobileController :
        BaseApiODataController<MixCmsContext, MixRelatedAttributeData>
    {
        public ApiODataRelatedAttributeSetDataMobileController(
        ...
        public async Task<ActionResult<MobileViewModel>> Details(string culture, string parentId, int parentType, string id)
        {
            ...
            else
            {
                model = new MixRelatedAttributeData()
                {
                    ...
                    ParentId = parentId,
                    Priority = MobileViewModel.Repository.Max(p => p.Priority).Data + 1
                };
            }
        ...
        public async Task<ActionResult<List<Lib.ViewModels.MixAttributeSetDatas.MobileViewModel>>> ListDataByParent(string culture, MixEnums.MixAttributeSetDataType parentType, string parentId, int? attributeSetId, ODataQueryOptions<MixRelatedAttributeData> queryOptions)
        {
            ...
            var result = await base.GetListAsync<MobileViewModel>(predicate, $"parent_{parentType}_{parentId}_{attributeSetId}", queryOptions);
            if (result != null)
            {
        ...
        [HttpPost, HttpOptions]
        [Route("{parentId}/{parentType}/{id}")]
        public async Task<ActionResult<MobileViewModel>> Save(string culture, string parentId, int parentType, string id, [FromBody]JObject data)
        {
            var portalResult = await base.SaveAsync<MobileViewModel>(data, p => p.Id == id && p.ParentId == parentId && p.ParentType == parentType && p.Specificulture == _lang);
            if (portalResult.IsSucceed)
            {
                return Ok(portalResult);
            }
        ...
        public async Task<ActionResult<MobileViewModel>> SaveProperties([FromBody]JArray data)
        {
            foreach (JObject item in data)
            {
                ...
                var portalResult = await base.SaveAsync<MobileViewModel>(properties, p => p.Id == id && p.ParentId == parentId && p.ParentType == parentType && p.Specificulture == _lang);
                if (!portalResult.IsSucceed)
                {
                    return BadRequest(portalResult);
                }
        ...
        [EnableQuery(MaxExpansionDepth = 4)]
        [HttpGet, HttpOptions]
        public async Task<ActionResult<List<MobileViewModel>>> List(string culture, ODataQueryOptions<MixRelatedAttributeData> queryOptions)
        {

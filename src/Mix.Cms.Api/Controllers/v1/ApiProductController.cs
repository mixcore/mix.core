...
using Mix.Cms.Lib.ViewModels.MixProducts;
...
namespace Mix.Cms.Api.Controllers.v1
{
    ...
    public class ApiProductController :
        BaseGenericApiController<MixCmsContext, MixProduct>
    {
        ...
        [HttpGet, HttpOptions]
        [Route("delete/{id}")]
        public async Task<RepositoryResponse<MixProduct>> DeleteAsync(int id)
        {
        ...
        public async Task<ActionResult<JObject>> Details(string viewType, int? id)
        {
            ...
            switch (viewType)
            {
                case "portal":
                    if (id.HasValue)
                    {
                        Expression<Func<MixProduct, bool>> predicate = model => model.Id == id && model.Specificulture == _lang;
                        var portalResult = await base.GetSingleAsync<UpdateViewModel>($"{viewType}_{id}", predicate);
                        if (portalResult.IsSucceed)
                        {
                            portalResult.Data.DetailsUrl = MixCmsHelper.GetRouterUrl("Product", new { portalResult.Data.SeoName }, Request, Url);
                        }
                ...
                default:
                    if (id.HasValue)
                    {
                        ...
                        {
                            beResult.Data.DetailsUrl = MixCmsHelper.GetRouterUrl("Product", new { beResult.Data.SeoName }, Request, Url);
                        }
        ...
        [HttpPost, HttpOptions]
        [Route("save/{id}")]
        public async Task<RepositoryResponse<MixProduct>> SaveFields(int id, [FromBody]List<EntityField> fields)
        {
            if (fields != null)
            {
                var result = new RepositoryResponse<MixProduct>() { IsSucceed = true };
            ...
            return new RepositoryResponse<MixProduct>();
        }
            ...
            [FromBody] RequestPaging request)
        {
            ...
            Expression<Func<MixProduct, bool>> predicate = model =>

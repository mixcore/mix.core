...
modules.component('tclLogin', {
    ...
    controller: ['$scope', '$rootScope', 'localStorageService', 'RestAttributeSetDataClientService',
        function ($scope, $rootScope, localStorageService, service) {
            var ctrl = this;
            ...
            ctrl.$onInit = async function () {
                $rootScope.user = localStorageService.get('tcl_user');
                if($rootScope.user){
            ...
            ctrl.onSuccess = function (result) {
                ...
                $rootScope.user = localStorageService.set('tcl_user', $rootScope.user);
                $scope.$apply();
            };
...
app.controller('PackageController', ['$scope', '$rootScope', 'ngAppSettings', 'localStorageService', 'RestAttributeSetDataClientService',
    function ($scope, $rootScope, ngAppSettings, localStorageService, service) {
        $scope.request = angular.copy(ngAppSettings.request);
        $scope.request.attributeSetName = 'tcl_package';
        $scope.packages = [];
        $scope.submitText = 'Chấp Nhận Đơn Hàng';
        $scope.init = async function () {
            $scope.user = localStorageService.get('tcl_user');

            if ($scope.user.obj.order_packages.length == 0) {
                await $scope.loadDefaultPackages();
                $scope.saveDefaultData().then(() => {
                    service.clearCache([$scope.user.obj.id]);
                    localStorageService.set('tcl_user', $scope.user);
                });
            }
            else {
                $scope.submitText = 'Cập Nhật Lại Đơn Hàng';
            }
            $scope.totalUnit = 0;
            angular.forEach($scope.user.obj.order_packages, function (pack) {
                $scope.calculateItems(pack);
                if (pack.obj.quantity > 0) {
                    $scope.totalUnit += (pack.obj.total * pack.obj.quantity);
                }
            });

        };

        $scope.loadDefaultPackages = async function () {
            $rootScope.isBusy = true;
            $scope.request.parentId = $scope.user.id;
            $scope.request.parentType = 1;
            var getPackages = await service.getList($scope.request);
            if (getPackages.isSucceed) {
                $scope.user.obj.order_packages = getPackages.data.items;
                angular.forEach($scope.user.obj.order_packages, function (pack) {
                    pack.parentId = $scope.user.id;
        ...
        $scope.saveDefaultData = async function () {
            angular.forEach($scope.user.obj.order_packages, async function (pack) {
                var savePackage = await $scope.saveData(pack);
                    ...
                    angular.forEach(pack.obj.package_slots, async function (slot) {
                        ...
                        if (saveSlot.isSucceed) {
                            ...
                            angular.forEach(slot.obj.package_items, async function (item) {
                                ...
                                var saveItem = await $scope.saveData(item);
        ...
        $scope.updatePackageQuantity = async function (pack, value) {
            const newVal = pack.obj.quantity + value;
            if (newVal < 0) {
                return;
            }
            pack.obj.quantity = newVal;
            $scope.totalUnit += (value * pack.obj.total);
            var result = await $scope.saveData(pack);
            $scope.handleResult(result);
            localStorageService.set('tcl_user', $scope.user);
            service.clearCache([$scope.user.obj.id]);
            // $scope.calculateItems();
        };
        $scope.activePackage = function (pack, $event) {
            if ($event.target.checked) {
                if (pack.obj.quantity == 0) {
                    pack.obj.quantity = 1;
                    $scope.totalUnit += (pack.obj.unit * pack.obj.quantity);
                }
            }
            else {
                if (pack.obj.quantity > 0) {
        ...
        $scope.updateItemQuantity = async function (pack, slot, item, value) {
            const newVal = item.obj.quantity + value;
            if (pack.obj.quantity == 0 || newVal < 0) {
                return;
            }
            if (!$scope.validateSlot(slot, value)) {
                $scope.alert(`${slot.obj.title} không thể chọn nhiều hơn ${slot.obj.total_unit} sản phẩm`)
                return;
            }
            if (item.obj.max_quantity >= newVal) {
                ...
                //     pack.obj.quantity = 0;
                // }
                var result = await $scope.saveData(item);
                $scope.handleResult(result);
                localStorageService.set('tcl_user', $scope.user);
            }
            else {
                $scope.alert(`Bạn không thể chọn nhiều hơn ${item.obj.max_quantity} sản phẩm ${item.obj.products[0].obj.title}`);
            }
        };

        $scope.calculateItems = function (pack) {
            pack.obj.total = 0;
            angular.forEach(pack.obj.package_slots, function (slot) {
                slot.total = 0;
                angular.forEach(slot.obj.package_items, function (item) {
                    slot.total += item.obj.quantity;
                    pack.obj.total += item.obj.quantity;
                });
                slot.isValid = slot.obj.total_unit == slot.total;
        ...
        $scope.submit = async function () {
            if ($scope.validate()) {
                $rootScope.isBusy = true;
                var result = await service.save($scope.user);
                $scope.handleResult(result);
                localStorageService.set('tcl_user', $scope.user);
            }
        };

        $scope.saveData = async function (data) {
        ...
        $scope.validateSlot = function (slot, value) {
            // let total = 0;
            // angular.forEach(slot.obj.package_items, function (item) {
            //     total += item.obj.quantity;
            // });
            let total = slot.total + value;
            slot.isValid = slot.obj.total_unit == total;
            return slot.obj.total_unit >= total;
        };
        ...
        $scope.validate = function () {
            ...
            angular.forEach($scope.user.obj.order_packages, function (pack) {
                if (!pack.isValid) {
                    result = false;
                    $scope.alert(`${pack.obj.title} chỉ chấp nhận ${pack.obj.unit} sản phẩm`);
                }
            });
            return result;
        }
        ...
        $scope.export = function () {
            if ($scope.validate()) {
                ...
                html2canvas(canvasdiv, {
                    ...
                    $.notify({
                        // options
                        message: 'TCL VIỆT NAM XIN CÁM ƠN QUÝ KHÁCH HÀNG ĐÃ ĐẶT HÀNG'
                    }, {
                        // settings
                        type: 'success',
                        placement: {
                    ...
                    a.download = $scope.user.obj.username + "_receipt_.png";
                    a.click();
                    $rootScope.isBusy = false;
                    $scope.$apply();
                });
        ...
        $scope.logout = function () {
            localStorageService.set('tcl_user', null);

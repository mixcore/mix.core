/* Sun May 31 2020 13:42:05 GMT+0700 (Indochina Time) */'use strict';
var app = angular.module('MixPortal', ['angularCroppie','ngRoute', 'components', 'ngFileUpload', 'LocalStorageModule',
    'bw.paging', 'dndLists', 'ngTagsInput', 'ngSanitize', 'ngMaterial']);
...
'use strict';
app.controller('HondaUserController', [
    '$scope', '$rootScope', '$location',
    'ngAppSettings', '$routeParams', 'HondaUserService',
    function ($scope, $rootScope, $location,
        ngAppSettings, $routeParams, service) {
        BaseRestCtrl.call(this, $scope, $rootScope, $routeParams, ngAppSettings, service);
        $scope.defaultAttr = null;
        $scope.actions = ['Delete'];
        $scope.request.orderBy = 'createdDateTime';
        $scope.getSingleSuccessCallback = function(){
            $scope.activedData.idCardDateObj =  new Date($scope.activedData.idCardDate);
        }
        $scope.validate = function(){
            $scope.activedData.idCardDate =  $scope.activedData.idCardDateObj.toISOString();
            return true;
        }
    }
]);
modules.component('hondaPointList', {
  bindings: {
    user: '='
  },
  templateUrl: '/app/app-portal/_custom/pages/honda-user/point-list/point-list.html',
  controller: ['$rootScope', '$scope', '$location', 'HondaPointService',
    function ($rootScope, $scope, $location, service) {
      ...
      ctrl.$onInit = async function () {
        ...
        if (getDefault.isSucceed) {
          ctrl.default = getDefault.data;
          ctrl.default.userid = ctrl.user.id;
          ctrl.point = angular.copy(ctrl.default);
        }
      }
      ...
      ctrl.save = async function () {
        $rootScope.isBusy = true;
        var saveResult = await service.save(ctrl.point);
        if (saveResult.isSucceed) {
          if (ctrl.point.id == 0) {
            ctrl.user.points.push(saveResult.data);
          }
          ctrl.point = angular.copy(ctrl.default);
          $rootScope.isBusy = false;
          $scope.$apply()
      ...
      ctrl.remove = function (id) {
        $rootScope.showConfirm(ctrl, 'removeConfirmed', [id], null, 'Remove', 'Deleted data will not able to recover, are you sure you want to delete this item?');
      };
      ...
      ctrl.removeConfirmed = async function (id) {
        ...
        var result = await service.delete([id]);
        if (result.isSucceed) {
          $rootScope.removeObjectByKey(ctrl.user.points, 'id', id);
          $rootScope.isBusy = false;
          $scope.$apply();
        } else {
          $rootScope.showMessage('failed');

...
namespace Mix.SignalR.Hubs
{
    public class EditFileHub : Hub
    {
        ...
        public async Task JoinRoom(string roomName, HubUserModel user)
        {
        ...
        public async Task LeaveRoom(string roomName, HubUserModel user)
        {
            if (Rooms.ContainsKey(roomName))
            {

                var users = Rooms[roomName] ?? new List<HubUserModel>();

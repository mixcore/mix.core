...
namespace Mix.Lib.Extensions
{
    public static class MixDatabaseDataValueModelExtensions
    {
        ...
        private static JArray GetRelatedData<T>(int referenceId, string dataId, string culture
            , MixCmsContext _context = null, IDbContextTransaction _transaction = null)
            where T: ViewModelBase<MixCmsContext, MixDatabaseDataAssociation, T>
        {
            UnitOfWorkHelper<MixCmsContext>.InitTransaction(
                    _context, _transaction,
                    out MixCmsContext context, out IDbContextTransaction transaction, out bool isRoot);
            Expression<Func<MixDatabaseDataAssociation, bool>> predicate = model =>
                    ...
                    ;
            var getData = DefaultRepository<MixCmsContext, MixDatabaseDataAssociation, T>.Instance.GetModelListBy(

/* Sat Apr 19 2025 18:13:35 GMT+0700 (Indochina Time) */"use strict";
...
app.controller("DashboardController", [
  ...
  function (
    ...
    $scope.initMqtt = function () {
      $scope.client = mqtt.connect("wss://mixcore.net:443/mqtt");
      $scope.topic = "iot/heart-rate";
      $scope.client.on("connect", () => {
        console.log("Connected to MQTT broker");
        $scope.initHeartChart();
        $scope.initHumanChart();
        $scope.client.subscribe($scope.topic, (err) => {
          if (!err) {
            console.log("Subscribed to iot/heart-rate");
          }
        });
      });
      $scope.client.on("message", (topic, message) => {
        // console.log("Received message from:", topic, message.toString());
        let jsonMessage = JSON.parse(message.toString());
        // const heartRate = parseInt(mess.toString());
        $scope.updateHeartChart(jsonMessage.heart.heart_phase);
        $scope.updateHumanChart(jsonMessage.human.targets);
      });
    };
    $scope.initHumanChart = function () {
      $scope.humanCtx = document.getElementById("humanChart").getContext("2d");

      // Data from your IoT device
      // Create the scatter chart
      $scope.humanChart = new Chart($scope.humanCtx, {
        type: "scatter",
        data: {
          datasets: [
            {
        ...
        options: {
          plugins: {
            tooltip: {
              ...
              position: "nearest", // Positioning of the tooltip

              callbacks: {
                title: function (tooltipItems) {
                  return `X: ${tooltipItems[0].parsed.x}, Y: ${tooltipItems[0].parsed.y}`;
                },
                label: function (tooltipItem) {
                  const moveSpeed = tooltipItem.raw.move_speed;
    ...
    $scope.initHeartChart = function () {
      $scope.heartCtx = document
        .getElementById("heartRateChart")
        .getContext("2d");
      $scope.heartRateChart = new Chart($scope.heartCtx, {
        type: "line",
        data: {
          labels: [], // Time labels
          datasets: [
    ...
    $scope.updateHeartChart = (heartRate) => {
      const now = new Date().toLocaleTimeString();
      $scope.heartRateChart.data.labels.push(now);
      $scope.heartRateChart.data.datasets[0].data.push(heartRate);
      $scope.heartRateChart.update();
    };
    $scope.updateHumanChart = (data) => {
      // $scope.humanData = data;
      // $scope.$apply();
      $scope.humanChart.data.datasets[0].data = data;
      $scope.humanChart.update();
    };
...
app.controller("MixDatabaseDataController", [
  ...
  function (
    ...
    $scope.initRouteParams = async () => {
      ...
      if ($routeParams.mixDatabaseName) {
...
app.controller("RoleController", [
  ...
  function (
    ...
    $scope.initPermissions = async function () {
      ...
      $scope.createUrl = `/admin/mix-database-data/create?mixDatabaseName=sysPermission&dataContentId=&parentId=${$scope.viewmodel.id}&parentName=Role&backUrl=${backUrl}`;
...
app.component("themeImportMixDatabases", {
  ...
  controller: [
    ...
    function ($rootScope, $scope, ngAppSettings) {
      ...
      ctrl.selectData = (mixDatabase, selected) => {
        ctrl.selectAllData = ctrl.selectAllData && selected;
        ctrl.importThemeDto.mixDatabaseDataIds = ctrl.updateArray(
          ctrl.importThemeDto.mixDatabaseDataIds,
          [mixDatabase.id],
          selected
        );
      };
  ...
  controller: [
    ...
    function (
      ...
      ctrl.addDefaultColumns = () => {
        ...
        ctrl.addColumn(
          ctrl.fieldNames.createdDateTime,
          "DateTime",
          true,
          "now()"
        );
        ctrl.addColumn(ctrl.fieldNames.lastModified, "DateTime", true, "now()");
  ...
  controller: [
    ...
    function (
      ...
      ctrl.loadData = async function (pageIndex, pagingState = null) {
        ...
        dataService.initDbName(ctrl.request.mixDatabaseName);
      ...
      ctrl.update = function (data) {
        let url = `/admin/mix-database-data/details?dataContentId=${
          data.id
        }&mixDatabaseName=${ctrl.request.mixDatabaseName}&mixDatabaseTitle=${
...
modules.component("mixDatabaseOneToMany", {
  ...
  controller: [
    ...
    function (
      ...
      ctrl.getParentIdColName = () => {
        return ctrl.database.namingConvention == "SnakeCase"
          ? `${ctrl.parentName}_id`
          : `${ctrl.parentNameId}`;
      };
      ctrl.toCamelCase = (txt) => {

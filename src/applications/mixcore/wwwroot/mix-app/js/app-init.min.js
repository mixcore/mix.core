  ...
  function (
    ...
    $scope.initCmsModel = {
      ...
      sqliteDbConnectionString:
        "Data Source=wwwroot\\mixcontent\\mix-cms.sqlite",
...
app.controller("Step3Controller", [
  ...
  function ($scope, $rootScope, apiService, storeService, service) {
    ...
    $scope.extractTheme = async function () {
      let theme = document.querySelector('input[type="file"][name="theme"]');
      ...
      var response = await service.extractTheme(frm, url);
      $rootScope.isBusy = false;
      if (response.success) {
...
app.factory("Step3Services", [
  ...
  function (apiService) {
    ...
    var _extractTheme = async function (form, url) {
      var req = {
        method: "POST",
        url: url,
        headers: { "Content-Type": undefined }, // Important for FormData
        transformRequest: angular.identity, // Important for FormData
    ...
    service.extractTheme = _extractTheme;
    return service;
  },
]);
...
app.controller("Step4Controller", [
  ...
  function ($scope, $rootScope, apiService, authService, service) {
    ...
    $scope.submit = async function (event) {
      $rootScope.isBusy = true;
      var result = await service.submit(event, $scope.importThemeDto);
      if (result.success) {
        window.top.location = "/";
      } else {
        if (result) {
...
app.factory("Step4Services", [
  ...
  function (apiService, commonService) {
    ...
    var _submit = async function (event, data) {
      var req = {
        method: "POST",
...
app.component("themeImportMixDatabases", {
  templateUrl:
    "/mix-app/views/app-init/pages/step4/components/theme-import-mix-db-tables/view.html",
  bindings: {
    importThemeDto: "=",
  },
  controller: [
    ...
    function ($rootScope, $scope, ngAppSettings, service) {
      ...
      ctrl.updateContent = function (arr, selected) {
        if (selected) {
          ctrl.importThemeDto.content.mixDbTableIds = ctrl.unionArray(
            ctrl.importThemeDto.content.mixDbTableIds,
            arr
          );
        } else {
          ctrl.importThemeDto.content.mixDbTableIds =
            ctrl.importThemeDto.content.mixDbTableIds.filter(
              (m) => arr.indexOf(m) < 0
            );
      ...
      ctrl.updateData = function (arr, selected) {
        if (selected) {
          ctrl.importThemeDto.data.mixDbTableIds = ctrl.unionArray(
            ctrl.importThemeDto.data.mixDbTableIds,
            arr
          );
        } else {
          ctrl.importThemeDto.data.mixDbTableIds =
            ctrl.importThemeDto.data.mixDbTableIds.filter(
              (m) => arr.indexOf(m) < 0
            );
        }
      };
